...
{
    public class LocalizationHandler : IHttpHandler, IRouteHandler
	{
		...
		public void ProcessRequest(HttpContext context)
		{
			...
			else if (context.Request.Url.AbsolutePath.Contains("/_localization/lang"))
			{
			    response = SetLanguage(context, context.Request.Params);
			}
			else
				response = R(context, GetResourcePath(context.Request.AppRelativeCurrentExecutionFilePath));

			context.Response.Write(response);
		}

        private CultureInfo GetCulture(NameValueCollection query)
        {
            return string.IsNullOrEmpty(query["culture"])
                ? DefaultCulture.Value
                : new CultureInfo(query["culture"]);
        }

        private string SetLanguage(HttpContext context, NameValueCollection query)
        {
            LocalizationManager.Instance.SetCulture(GetCulture(query));
            context.Response.Redirect(context.Request.UrlReferrer == null ? "/" : context.Request.UrlReferrer.ToString(), true);
            return "";
        }
        ...
        private string Api(HttpContext context, string endpoint, NameValueCollection query)
		{
			...
			switch (endpoint)
			{
				...
				case "tree":
					var resources = _manager.GetAll(GetCulture(query));
					response = serializer.Serialize(
		...
		private string R(HttpContext context, string path)
		{
			...
			switch (Path.GetExtension(path))
			{
				...
				case ".png":
					...
					Image.FromStream(GetResourceStream(path)).Save(context.Response.OutputStream, ImageFormat.Png);
					break;
				case ".gif":
					response.ContentType = "image/gif";
					Image.FromStream(GetResourceStream(path)).Save(context.Response.OutputStream, ImageFormat.Gif);
					break;
				case ".html":
		...
		{
			var path = "Lion.Localization.Resources.";
