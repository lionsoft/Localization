...
namespace Lion.Localization.Mvc
{
	...
	{
		public string L(string text, params object[] formatterArguments)
		{
			return this.Translate(text, formatterArguments: formatterArguments);
		}

		public string L(string text, CultureInfo culture, params object[] formatterArguments)
		{
			return this.Translate(text, culture, formatterArguments);
		}

		public HtmlString L2(string text, params object[] formatterArguments)
		{
			return this.TranslateMarkup(text, formatterArguments: formatterArguments);
		}

		public HtmlString L2(string text, CultureInfo culture, params object[] formatterArguments)
		{
	    ...
	    public MvcHtmlString RenderLocalesList(string itemTemplate = null, string wrapperTemplate = "")
	    {
            itemTemplate = itemTemplate ?? "<li><a href='/_localization/lang?culture={0}'>{1}</a></li>";
	        return RenderLocalesList(c => string.Format(itemTemplate, c.Name, c.DisplayName), wrapperTemplate);
	    }

	    public MvcHtmlString RenderLocalesList(Func<CultureInfo, string> onGetItem, string wrapperTemplate = "")
	    {
            var sb = new StringBuilder();
            var cultures = LocalizationManager.Instance.GetCultures().ToArray();
            if (cultures.Length > 1)
            {
                foreach (var culture in cultures)
                {
                    sb.AppendLine(onGetItem(culture));
                }
            }
            var res = sb.ToString();
            if (!string.IsNullOrWhiteSpace(wrapperTemplate))
                res = string.Format(wrapperTemplate, res);
	...
	internal static class StringExtensions
	{
		internal static string ParseMarkup(this string value)
		{
			...
			foreach (Match match in regex.Matches(result))
			{
				...
				var tagBuilder = new StringBuilder("<a");
				if (items.Length > 1)
				{
